    ...
    while(True):
        ...
        if (user_choice=="A"or user_choice=="a"):
            ...
            for image_in_file in os.listdir(path):
                ...
                #try and except function to check if the image_name exists in the folder and can be encoded
                try:
                    image = cv2.imread(image_in_file)
                except:
                    print("Image could not be found to encode, Please enter correct image name or check location")
                    add_sample()
                rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        ...
        if inputer=="N":
            break
    # dump the facial encodings + expressions to disk
    data = {"encodings": encodings, "expression": expression}
...
def report_bug():
    #displays messages to use informing of the page
    print("####################################")
    print("Welcome to bug repoting")
    print("We are sorry you have discoved a bug")
    #takes the user's input
    bug=input("Please list the issue here :")
    #opens and appends to the text file
    bug_file=open("BUG_REPORT","w")
